...
**Ejercicio 1.** Comparando `s² + 4s + 25` con `s² + 2ζωn·s + ωn²`: `ωn = √25 = 5 rad/s` y `2ζωn = 4`, luego `ζ = 0.4`. Sobreoscilación `e^(−π·0.4/√(1−0.16))·100 = 25.4 %`, y `ts ≈ 4.6/(ζ·ωn) = 4.6/2 = 2.3 s`. `ct.step_info` devuelve `Overshoot ≈ 25.3 %` —la fórmula de la sobreoscilación es exacta— y `SettlingTime ≈ 1.71 s`, sensiblemente menor que la regla. La razón: la regla `4.6/(ζωn)` está construida sobre la **envolvente** exponencial `e^(−ζωn·t)`, y da el instante en que la envolvente entra en la banda del 1 %; la señal real puede entrar antes, porque el coseno que la modula pasa por cero. La regla es conservadora, que es como debe ser una regla de diseño.
